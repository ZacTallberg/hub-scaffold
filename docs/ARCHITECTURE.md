...
| Atomic pull routes only compatible work | The canonical ready frontier is derived first; declared capability, risk, resource, locality, and outcome constraints filter incompatible tasks before soft quality/latency/cost ranking | A per-pull worker profile is a placement declaration, not durable identity. Missing facts cannot satisfy explicit constraints; unconstrained tasks retain the zero-profile path. |

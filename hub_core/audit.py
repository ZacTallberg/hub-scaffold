...
def oracle_tamper_selftest():
    """Both directions against a REAL scratch git repo: a commit deleting an assertion from the
    vc-named test FIRES critical; a clean feature commit stays quiet. Returns 0 iff both hold."""
    import subprocess
    import tempfile
    from pathlib import Path

    repo = Path(tempfile.mkdtemp(prefix="oracle_tamper_st_"))
    ...
    g("init")
    g("config", "user.name", "oracle-selftest")
    g("config", "user.email", "st@local")
    (repo / "tests").mkdir()
    ...
    # tamper commit: delete an assertion from the oracle's test file
    (repo / "tests" / "test_thing.py").write_text(
        "def test_thing():\n    assert True\n", encoding="utf-8")
    g("add", "-A"); g("commit", "-m", "tamper")
    tamper_sha = g("rev-parse", "HEAD")

    # clean commit: a feature change that never touches the oracle
    (repo / "feature.py").write_text("def f():\n    return 2\n", encoding="utf-8")
    g("add", "-A"); g("commit", "-m", "clean feature")
    clean_sha = g("rev-parse", "HEAD")

    # rename-recreate SPLIT across two commits: shove the oracle aside untouched (a pure rename,
    # quiet on its own diff), then mint a hollow file at its path in the next commit. Only the
    # per-task pooled pre-image sees the pair as one edit — this was the exemption bypass.
    g("mv", "tests/test_thing.py", "tests/test_thing_old.py")
    g("commit", "-m", "rename away")
    rename_sha = g("rev-parse", "HEAD")
    (repo / "tests" / "test_thing.py").write_text(
        "import sys\ndef test_thing():\n    sys.exit(0)\n", encoding="utf-8")
    g("add", "-A"); g("commit", "-m", "recreate hollow")
    recreate_sha = g("rev-parse", "HEAD")

    # authorship control: a task birthing its OWN brand-new oracle (exit-0 plumbing and all,
    # no rename anywhere in its commit set) must stay quiet
    (repo / "tests" / "test_new_oracle.py").write_text(
        "import sys\n\ndef test_new():\n    assert 2 + 2 == 4\n\n"
        "if __name__ == '__main__':\n    test_new(); sys.exit(0)\n", encoding="utf-8")
    g("add", "-A"); g("commit", "-m", "author new oracle")
    authored_sha = g("rev-parse", "HEAD")

    def state_for(*shas, vc="python tests/test_thing.py"):
        return {"by_type": {
            "task": [{"id": "x:task:probe", "type": "task", "status": "done",
                      "work_kind": "product",
                      "verification_command": vc}],
            "commit": [{"type": "commit", "task": "x:task:probe", "sha": s} for s in shas]}}

    fired = oracle_tamper_adapter(state_for(tamper_sha), repo_dir=repo)
    assert fired and fired[0]["severity"] == "critical" and "net-removed" in fired[0]["observed"], \
        f"an assertion-deleting commit must fire critical, got {fired}"
    quiet = oracle_tamper_adapter(state_for(clean_sha), repo_dir=repo)
    assert not quiet, f"a clean feature commit must stay quiet, got {quiet}"
    recreated = oracle_tamper_adapter(state_for(rename_sha, recreate_sha), repo_dir=repo)
    assert recreated and recreated[0]["severity"] == "critical" \
        and "exit-0 short-circuit" in recreated[0]["observed"], \
        f"a rename-recreated oracle is an edit, not authorship — must fire critical, got {recreated}"
    authored = oracle_tamper_adapter(
        state_for(authored_sha, vc="python tests/test_new_oracle.py"), repo_dir=repo)
    assert not authored, f"a task authoring its own brand-new oracle must stay quiet, got {authored}"
    print("ORACLE-TAMPER SELFTEST PASS: assertion-deleting commit fires critical; clean feature "
          "commit stays quiet; split rename-recreate fires critical; new-oracle authorship quiet.")
    return 0
...
def spin_guard_selftest():
    """Both directions over synthetic per-(worker,task) trajectories: k consecutive failed-finishes
    AND an A-B-A-B oscillation FIRE (warn, naming the task); a healthy trajectory (fails then
    advances, or below k) stays quiet; a frontier advance inside the window resets the run."""
    def vf(worker, task):    # a failed-finish event
        return {"type": "verification.recorded", "agent_id": worker,
                "payload": {"task": task, "exit_code": 1}}

    def vo(worker, task):    # an exit-0 verification (advance)
        return {"type": "verification.recorded", "agent_id": worker,
                "payload": {"task": task, "exit_code": 0}}

    def ts(worker, task, status):   # a task status event
        return {"type": "task.updated", "aggregate": task, "agent_id": worker,
                "payload": {"status": status}}

    def te(worker, task, **fields):   # a field-only task edit (no status change)
        return {"type": "task.updated", "aggregate": task, "agent_id": worker, "payload": fields}

    W, T = "worker-x", "demo:task:spin"
    def ids(v):
        return {x["id"] for x in v}

    # 1 - k consecutive failed-finishes fire (warn, naming the task)
    v = spin_guard_adapter([vf(W, T) for _ in range(4)])
    assert ids(v) == {"coherence:worker-spin:spin"}, f"4 consecutive fails must fire, got {ids(v)}"
    assert all(x["severity"] == "warn" for x in v), "worker-spin is amber (warn), never blocking"
    assert T in v[0]["observed"] and W in v[0]["observed"], "the violation must name the (worker, task)"

    # 2 - A-B-A-B oscillation fires (claim active <-> release todo, never advancing)
    osc = [ts(W, T, "active"), ts(W, T, "todo"), ts(W, T, "active"), ts(W, T, "todo")]
    assert ids(spin_guard_adapter(osc)) == {"coherence:worker-spin:spin"}, "A-B-A-B oscillation must fire"

    # 3 - below k stays quiet (k-1 fails)
    assert not spin_guard_adapter([vf(W, T) for _ in range(3)]), "3 (< k) fails must stay quiet"

    # 4 - a healthy trajectory: fails then ADVANCES (exit-0) resets the run -> quiet
    healthy = [vf(W, T), vf(W, T), vf(W, T), vo(W, T)]
    assert not spin_guard_adapter(healthy), "fails then an exit-0 advance is healthy, must stay quiet"

    # 5 - a resolving oscillation that reaches done is not a spin (advance at the end resets)
    resolved = [ts(W, T, "active"), ts(W, T, "todo"), ts(W, T, "active"), ts(W, T, "done")]
    assert not spin_guard_adapter(resolved), "an oscillation that reaches done must stay quiet"

    # 6 - two DIFFERENT workers each below k on the same task stay quiet (spin is per-worker)
    split = [vf("w1", T), vf("w1", T), vf("w2", T), vf("w2", T)]
    assert not spin_guard_adapter(split), "two workers each < k must stay quiet (spin is per (worker,task))"

    # 7 - three+ distinct alternating states = exploring, not stuck -> quiet
    explore = [ts(W, T, "active"), ts(W, T, "todo"), ts(W, T, "blocked"), ts(W, T, "active")]
    assert not spin_guard_adapter(explore), "three distinct states is exploration, not a spin"

    # 8 - k consecutive fails followed by a status touch STILL fire (identical scanned across the
    #     whole run, not just the trailing window) — the false-negative the verifier caught
    assert ids(spin_guard_adapter([vf(W, T)] * 4 + [ts(W, T, "active")])) == {"coherence:worker-spin:spin"}, \
        "4 consecutive fails then a status touch must still fire (identical is not masked by a trailing event)"

    # 9 - FALSE-POSITIVE fixes: a worker ENRICHING a task it is working must stay quiet
    #     9a - field-only edits (no status change) carry no spin signal
    edits = [te(W, T, evidence_uri=["e"]), te(W, T, deps=["d"]), te(W, T, note="n"), te(W, T, priority="P1")]
    assert not spin_guard_adapter(edits), "field-only edits (no status) must not fire — a worker enriching a task"
    #     9b - repeated same-status re-saves are deduped (not transitions)
    assert not spin_guard_adapter([ts(W, T, "active")] * 5), "same-status re-saves must not fire"
    #     9c - forward working-state oscillation (active<->in_progress, active<->blocked) is not a spin
    assert not spin_guard_adapter([ts(W, T, "active"), ts(W, T, "in_progress")] * 3), \
        "active<->in_progress (async round-trips) must not fire — a forward working oscillation"
    assert not spin_guard_adapter([ts(W, T, "active"), ts(W, T, "blocked")] * 3), \
        "active<->blocked (dep-gated waiting) must not fire — the worker is parked, not looping"

    # 10 - claim/release CHURN (active<->todo) and a start->fail retry loop DO fire (regress present)
    assert ids(spin_guard_adapter([ts(W, T, "active"), ts(W, T, "todo")] * 2)) == {"coherence:worker-spin:spin"}, \
        "active<->todo reclaim churn must fire (todo is a regress/release)"
    assert ids(spin_guard_adapter([ts(W, T, "active"), vf(W, T)] * 2)) == {"coherence:worker-spin:spin"}, \
        "claim->fail->claim->fail retry loop must fire"

    print("SPIN-GUARD SELFTEST PASS: k consecutive fails (even with a trailing status touch) and "
          "claim<->fail / claim<->release churn fire the amber coherence:worker-spin naming the "
          "(worker,task); below-k, fails-then-advance, resolved-to-done, split-workers, three-state "
          "exploration, field-only edits, same-status re-saves, and forward working-state "
          "oscillations (active<->in_progress / active<->blocked) all stay quiet.")
    return 0
